#Código para baixar dados brutos do TCGA/GDC e para criar uma samplesheet utilizável para o pipeline do ChAMP.
#A samplesheet eu baixei do repositório do GDC.
# segui o passo a passo deste link aqui: https://cloud.tencent.com/developer/article/2163267
...
query_TCGAidats <- GDCquery(project = 'TCGA-HNSC',
...
View(getResults(query_TCGAidats))
getwd()
setwd('/data04/projects04/SheilaCoelho/headspace_methyl/data/idats_tcga_larynx_tumors/')

#Baixar os idats
GDCdownload(query = query_TCGAidats)


#Mover arquivos idat para uma pasta só

# Listar todos os subdiretórios dentro de 'Masked_Intensities'
subdirectories <- list.dirs(full.names = FALSE, recursive = FALSE)
#260 pastas

getwd()
# Loop through each subdirectory
for (subdir in subdirectories) {
  # List all files in the subdirectory with the .idat extension
  idat_files <- list.files(path = subdir, pattern = "\\.idat$", full.names = TRUE)
  
  # Move the .idat files to the parent directory ('Masked_Intensities')
  file.copy(from = idat_files, to = file.path("/data04/projects04/SheilaCoelho/headspace_methyl/data/idats_tcga_larynx_tumors"))
  
  # Optionally, you can uncomment the line below to delete the original files after copying
  # file.remove(idat_files)
}


#Checar quantos arquivos eu tenho de cada .idat
GrnPath <- unlist(sapply(paste(pd1$Sentrix_ID,pd1$Sentrix_Position,"Grn.idat",sep="_"), 
                         function(x) grep(x,list.files( "/data04/projects04/SheilaCoelho/headspace_methyl/data/idats_tcga_larynx_tumors",
                                                        recursive=T,full.names=TRUE), value = TRUE)))
GrnPath


RedPath <-unlist(sapply(paste(pd1$Sentrix_ID,pd1$Sentrix_Position,"Red.idat",sep="_"), 
                        function(x) grep(x,list.files( "/data04/projects04/SheilaCoelho/headspace_methyl/data/idats_tcga_larynx_tumors",
                                                       recursive=T,full.names=TRUE), value = TRUE)))
RedPath
...
#Sample sheet que baixei do site do GDC (dei upload no R)
sample_sheet <- read.table("gdc_sample_sheet.2024-02-01.tsv",
                           sep = "\t",
                           header = T)

#Transformar em formato pd file
...
write.csv(pd1,file = "/data04/projects04/SheilaCoelho/headspace_methyl/data/idats_tcga_larynx_tumors/sample_sheet.csv",quote = F,row.names = F)
